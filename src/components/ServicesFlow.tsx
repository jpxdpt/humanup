...
export function ServicesFlow() {
  ...
  const { hero, packagesTitle, packages, methodTitle, methodSteps, cta } = servicos;
    ...
    <div>
      ...
      <section className="relative flex flex-col justify-center items-center w-full min-h-screen overflow-hidden bg-on-surface text-white">
        <div className="absolute inset-0">
          <div
            ...
            style={{ backgroundImage: "url('/images/sigmund-HKr9cdfrbOo-unsplash-1024x683.jpg')" }}
          />
      ...
      <section className="relative w-full bg-primary-container text-on-primary-container py-24 md:py-32 overflow-hidden">
        ...
        <Reveal className="container-site max-w-[960px]">
          ...
          <Reveal direction="up" distance={24} delay={100}>
            <h2 className="text-display text-center mb-6 capitalize">{methodTitle}</h2>
          </Reveal>
            ...
            {methodSteps.map((step, i) => (
              <Reveal key={i} direction="up" distance={24} delay={200 + i * 80}>
                <div className="text-center">
                  <span className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-on-primary-container/10 text-on-primary-container text-2xl font-bold mb-6 ring-1 ring-on-primary-container/20">
                    {step.number}
                  </span>
                  <h3 className="text-2xl md:text-3xl font-bold mb-3 leading-tight">{step.title}</h3>
                  <p className="font-sans text-base font-medium leading-relaxed text-on-primary-container/75">
                    {step.description}
                  </p>
                </div>
              </Reveal>
      ...
      <section className="relative w-full bg-surface text-on-surface py-24 md:py-32 overflow-hidden">
        ...
        <Reveal className="container-site">
          ...
          <Reveal direction="up" distance={24} delay={100}>
            <h2 className="text-display text-center mb-16 capitalize">{packagesTitle}</h2>
          </Reveal>
